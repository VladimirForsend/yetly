    ...
    <section className="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-card" aria-labelledby="confidence-heading">
      <div className="bg-gradient-to-r from-brand-50 via-white to-blue-50 p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label><span className="text-xs font-black text-ink-600">Modelo</span><select value={model} onChange={(event) => selectModel(event.target.value)} disabled={loadingModels || !models.length} className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold outline-none disabled:bg-slate-100 focus:border-brand-500 focus:ring-4 focus:ring-brand-100">{loadingModels && <option>Cargando modelos…</option>}{models.map((item) => <option key={item.model} value={item.model}>{item.name}{item.model.toLowerCase().startsWith("gemma") ? " · recomendado" : ""}{item.capabilities.includes("tools") ? " · propuestas" : " · análisis"}</option>)}</select></label>
        </div>
